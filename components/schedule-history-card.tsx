...
function ScheduleHistoryCard({ schedule }: { schedule: ScheduleList }) {
  ...
  return (
    <div>
        <Card onClick={handleScheduleClick} 
        className='border border-primary/30 bg-primary/10 hover:bg-primary/35 flex p-4 px-6 justify-between items-center cursor-pointer'>
            <CardTitle>
                Schedule - {schedule.id}
            </CardTitle>
            <h1>Semester - {schedule.semester}</h1>
            <CardDescription>
                Generated on: {new Date(schedule.createdAt).toLocaleDateString(undefined, {
                year: 'numeric',
                month: 'short',
                day: 'numeric',
