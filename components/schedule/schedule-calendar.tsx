...
export default function ScheduleCalendar({
    ...
    const [localScheduleList, setLocalScheduleList] = useState<
    ...
    return (
        <div className={cn(className, 'bg-transparent')}>
            <FullCalendar
                ...
                eventAllow={(dropInfo) => {
                    const startTime = dropInfo.start.toLocaleTimeString(
                        'en-GB',
                        {
                            hour: '2-digit',
                            minute: '2-digit',
                ...
                eventOverlap={(a, b) => {
                    ...
                    const roomA =
                        a.extendedProps.room?.name || a.extendedProps.room;
                    const roomB =
                        b?.extendedProps.room?.name || b?.extendedProps.room;

                    const lecturerA =
                        a.extendedProps.lecturer?.name ||
                        a.extendedProps.lecturer;
                    const lecturerB =
                        b?.extendedProps.lecturer?.name ||
                        b?.extendedProps.lecturer;
                    // Return true to allow overlap, false to prevent it
                    if (roomA == roomB)
                        toast.error('Overlapping schedule with the same room!');
                    if (lecturerA == lecturerB)
                        toast.error(
                            'Overlapping schedule with the same lecturer!',
                        );
